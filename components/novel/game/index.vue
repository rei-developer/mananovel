...
export default {
  ...
  mounted() {
    ...
    this.$eventBus.$on('stopBGM', () => this.stopBGM())
    this.$eventBus.$on('stopBGS', () => this.stopBGS())
  },
  beforeDestroy() {
  ...
  methods: {
    async show(id, version, isDebug = false) {
      ...
      this.novel = novel
      if (this.novel.isUseTitle === 1)
        this.isUseTitle = true
      if (this.novel.isUseCustomCSS)
