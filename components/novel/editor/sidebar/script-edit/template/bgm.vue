...
export default {
  name: 'NovelEditorSidebarScriptEditTemplateBgm',
  components: {
    EInput,
    ESelect,
    EButton
  ...
  data() {
    ...
    const attr = {}
    attrList.map(item => attr[item] = !!data.bgm ? data.bgm[item] : undefined)
    return {
      data,
      ...attr,
  ...
  watch: {
    pureData() {
      ...
      attrList.map(item => this[item] = !!this.pureData.bgm ? this.pureData.bgm[item] : undefined)
    }
  },
  computed: {
    isAllEmpty() {
  ...
  methods: {
    ...
    onClickSave() {
      if (this.isAllEmpty)
        ...
        this.data.bgm = {}
        attrList.map(item => this[item] ? this.data.bgm[item] = this[item] : undefined)
      }
      this.submit()
    },
    submit() {
